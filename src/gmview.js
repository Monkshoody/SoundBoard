...
import { loadPermissions, savePermissions, saveSoundData, loadSoundData, loadSoundPermissions, saveSoundPermissions, playSoundForAll, playSoundForPlayer, triggerGlobalNotification } from "./permissions.js";
...
// initiate metadata for the OwlBear namespace
  ...
  let permissionsKey = await loadSoundPermissions();
...
// audio-toggle container
  ...
  checkbox.addEventListener("change", async () => {
    await saveSoundPermissions(!checkbox.checked);
  });
      ...
      case "import-permissions":
        ...
        inputPermissions.onchange = async (e) => {
          ...
          try {
            ...
            // if newPermissions contain permissions of a sound which is not in the soundData remove that sound from the imported Data
            const availableSounds = soundData.map(s => s.name); // Liste der verfügbaren Sound-Namen
            const cleanedPermissions = {};
            for (const player in newPermissions) {
              const permittedSounds = newPermissions[player].filter(sound => availableSounds.includes(sound));
              if (permittedSounds.length > 0) {
                cleanedPermissions[player] = permittedSounds;
              }
            }
